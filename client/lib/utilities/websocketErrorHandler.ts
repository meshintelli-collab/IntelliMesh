...
class WebSocketErrorHandler {
  ...
  private handleWebSocketError(event: Event | PromiseRejectionEvent | Error) {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.warn(
        "Max websocket reconnection attempts reached. Page will continue to work without HMR.",
      );
      return;
    }
  ...
  private attemptReconnection() {
    ...
    setTimeout(() => {
      try {
        ...
        console.warn(
          "HMR reconnection failed, but app will continue to work:",
          error,
        );
      }
