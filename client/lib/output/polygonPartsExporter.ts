...
`;
  ...
  /**
   * Detect if polygon merging should be avoided to prevent windmilling
   * Returns true for complex shapes that are likely to cause artifacts
   */
  private static shouldAvoidPolygonMerging(
    mergedFaces: any[],
    geometry: THREE.BufferGeometry
  ): boolean {
    ...
    // High compression ratio suggests complex merging that may cause windmilling
    if (compressionRatio > 10) {
      console.log(`   🚨 HIGH compression ratio detected (${compressionRatio.toFixed(1)}x) - likely to cause windmilling`);
      return true;
    }

    // Check for polygons with many vertices (likely from over-merging)
    for (const face of mergedFaces) {
      if (face.vertices && face.vertices.length > 8) {
        console.log(`   🚨 Complex polygon detected (${face.vertices.length} vertices) - likely to cause windmilling`);
        return true;
      }
    }

    console.log(`   ✅ Shape appears simple enough for polygon merging`);
    return false;
  }
