...
export class FaceExtruder {
  ...
  static extrudeFace(face: Face, thickness: number): ExtrudedFace {
    ...
    console.log(
      `   📊 Input normal:`,
      `(${face.normal.x.toFixed(3)}, ${face.normal.y.toFixed(3)}, ${face.normal.z.toFixed(3)})`,
    );
    ...
    const backVertices = face.vertices.map((v) =>
      v.clone().add(extrusionOffset),
    );
    const backFace: Face = {
  ...
  static extrudedFaceToOBJ(
    ...
    objContent += this.faceToOBJPolygon(
      extrudedFace.frontFace.vertices.length,
      1,
      1,
    );
    ...
    objContent += this.faceToOBJPolygon(
      extrudedFace.backFace.vertices.length,
      backStartIndex,
      2,
    );
  ...
  private static faceToSTLTriangles(face: Face): string {
    ...
    if (face.vertices.length === 3) {
      // Already a triangle
      stlContent += this.addSTLTriangle(
        face.vertices[0],
        face.vertices[1],
        face.vertices[2],
        face.normal,
   ...
   */
  private static faceToOBJPolygon(
    vertexCount: number,
    startIndex: number,
    normalIndex: number,
  ): string {
